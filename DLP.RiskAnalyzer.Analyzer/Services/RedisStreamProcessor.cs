...
public class RedisStreamProcessor : IRedisStreamProcessor
{
    ...
    public async Task<int> ProcessRedisStreamAsync()
    {
        ...
        foreach (var phase in phases)
        {
            ...
            var phaseLimit = phase == "0" ? RedisProcessorConstants.PendingPhaseMaxBatches : RedisProcessorConstants.NewPhaseMaxBatches; // Max batches per phase
        ...
        if (pendingInserts.Count > 0)
        {
            ...
            if (saved > 0) _logger.LogDebug("Batch saved {Count} new incidents", saved);
            if (skipped > 0) _logger.LogWarning("Batch skipped {Count} duplicate/error incidents", skipped);
        }
