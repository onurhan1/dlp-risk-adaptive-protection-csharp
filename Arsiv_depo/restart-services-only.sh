...
# Restart Services Only (without Docker)
# For when PostgreSQL and Redis are already running
...
echo -e "${CYAN}=== Restarting DLP Risk Analyzer Services (No Docker) ===${NC}"
echo -e "${YELLOW}Note: PostgreSQL and Redis must be running separately${NC}"
echo ""
...
echo -e "${CYAN}[Step 2/4] Checking PostgreSQL...${NC}"
if command -v psql &> /dev/null; then
    if psql -U postgres -h localhost -d dlp_analyzer -c "SELECT 1;" > /dev/null 2>&1; then
...
# Step 3: Check Redis
echo -e "${CYAN}[Step 3/4] Checking Redis...${NC}"
...
echo -e "${CYAN}[Step 4/4] Starting Analyzer API (http://0.0.0.0:5001)...${NC}"
...
echo -e "${CYAN}[Step 5/5] Starting Collector Service...${NC}"
...
echo -e "  • Analyzer API:     ${CYAN}http://localhost:5001${NC}"
if [ ! -z "$SERVER_IP" ]; then
    echo -e "  • Analyzer API:     ${CYAN}http://$SERVER_IP:5001${NC} (network)"
fi
...
if [ ! -z "$SERVER_IP" ]; then
    echo -e "  • Dashboard:        ${CYAN}http://$SERVER_IP:3002${NC} (network)"
fi
echo ""
