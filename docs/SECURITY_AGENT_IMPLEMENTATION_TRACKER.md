...
| 2026-09-20 | 15 | Tamamlandi | Guvenlik Agent'i, Laboratuvar ile ayni kayitli ayarlar uzerinden manuel baglanti testi sunar. Kapali/erisilemeyen cihaz, bulunamayan model veya endpoint, timeout, modelin bos ya da gecersiz yaniti ayri durum ve onerilen aksiyonla gorunur. Ollama `done_reason=length` sinyali sohbet yanitinda kesilme uyarisi olarak gosterilir. Agent sohbeti ve workflow taslagi hatalari da genel 502 yerine bu ayrintili durumlari dondurur. Swagger'a JWT Bearer semasi eklenerek yetkili test istegi arayuzden de yapilabilir. `dotnet build --no-restore` ve `npx tsc --noEmit` basarili; commit var, push kullanici talimati bekliyor. |
