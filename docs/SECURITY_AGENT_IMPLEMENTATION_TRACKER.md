...
| 15 | Model sagligi ve cikti tamligi | Agent ekraninda yerel model baglanti testi, ayrintili hata siniflandirmasi ve kesilen model ciktilarinin gorunur uyarisi. | Tamamlandi |
| 16 | Dogal dil donem baglami | Laboratuvarda ozet/rapor niyetinin olay baglamiyla calismasi, yazili goreli tarihlerin ayrismasi ve sohbetin secili baglami korumasi. | Tamamlandi |
| 17 | Shadow degerlendirme kaydi | Shadow Risk analist geri bildiriminin API JSON sozlesmesine uygun gonderilmesi. | Tamamlandi |
...
| 2026-09-21 | 15 | Tamamlandi | Guvenlik Agent'i, Laboratuvar ile ayni kayitli ayarlar uzerinden manuel baglanti testi sunar. Kapali/erisilemeyen cihaz, bulunamayan model veya endpoint, timeout, modelin bos ya da gecersiz yaniti ayri durum ve onerilen aksiyonla gorunur. Qwen3'un `thinking` alanini uretip `done_reason=length` ile nihai `response` uretmeden bitirmesi `thinking_budget_exhausted` olarak ayristirilir. Laboratuvar ve Agent istekleri `think:false` ile nihai cevabi ister; saglik testi 256 tokenlik emniyet payi kullanir. Ollama `done_reason=length` sinyali sohbet yanitinda kesilme uyarisi olarak gosterilir. Agent sohbeti ve workflow taslagi hatalari da genel 502 yerine bu ayrintili durumlari dondurur. Swagger'a JWT Bearer semasi eklenerek yetkili test istegi arayuzden de yapilabilir. `dotnet build --no-restore` ve `npx tsc --noEmit` basarili; commit var, push kullanici talimati bekliyor. |
| 2026-09-21 | 16 | Tamamlandi | “Son 1 haftayi ozetle” gibi ozet/rapor/listeleme ifadeleri Laboratuvarda olay analizi olarak ayrisir; UI'da secili baslangic-bitisi yoksa son 1 hafta/ay veya son N gun/hafta/ay yazili olarak ayrisip gercek snapshot'a uygulanir. Secili UI tarihleri her zaman onceliklidir. Salt sohbetin bos snapshot'i arayuzde yuklenmis olay baglamini artik ezmez. Dogrulama ve commit var, push kullanici talimati bekliyor. |
| 2026-09-21 | 17 | Tamamlandi | Shadow Risk panelindeki analist degerlendirmesi, sunucunun snake_case JSON sozlesmesine uygun `user_email` alanini gonderir. Onceki camelCase alan sunucuda bos baglanip kaydin reddedilmesine yol aciyordu. TypeScript kontrolu ve commit var, push kullanici talimati bekliyor. |
